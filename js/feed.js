...
import { ownerRowHtml } from "./badges.js";

const PROFILE_FIELDS = "username, is_admin, is_mod, is_verified, avatar_url";
...
async function renderSingleVideo(feedEl, username, videoId) {
  const { data: v, error } = await supabase
    .from("videos")
    .select(`id, cf_stream_uid, caption, l_count, comment_count, created_at, owner_id, status, profiles(${PROFILE_FIELDS})`)
    .eq("id", videoId)
    .single();
...
async function loadMore(feedEl) {
  ...
  const { data: videos, error } = await supabase
    .from("videos")
    .select(`id, cf_stream_uid, caption, l_count, comment_count, created_at, owner_id, profiles(${PROFILE_FIELDS})`)
    .eq("status", "ready")
...
function renderSlide(v, liked) {
  ...
  slide.innerHTML = `
    <div class="slip">
      ...
      <div class="slip-body">
        <div class="slip-owner">FILED BY ${ownerRowHtml(v.profiles)}</div>
